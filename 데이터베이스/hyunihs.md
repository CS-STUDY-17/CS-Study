...
![images-emplam27-post-ddbae2c9-da94-457d-bad8-77ff6791255b-B트리 기본 형태](https://github.com/CS-STUDY-17/CS-Study/assets/86969518/3b1d2ea4-4f21-428f-9836-8cf6f8fd64f4)

> 인덱스가 효율적인 이유
> - 효울적인 단계를 거쳐 모든 요소에 접근할 수 있는 균형 잡힌 트리구조 
> - 트리 깊이의 대수 확장성
>   - 대수 확장성: 트리 깊이가 리프 노드 수에 비해 매우 느리게 성장
>   - 트리 깊이 +1 될수록 인덱스 항목은 기하급수적으로 많아짐

### 인덱스 만드는 방법
#### MySQL
- 클러스터형 인덱스 
  - primary key 옵션으로 기본키 만들때
  - 기본키로 만들지 않고 unique not null 옵션 붙이면
- 세컨더리 인덱스
  - create index ... 명령어로 생성
- 하나의 인덱스만 생성할 것이라면 클러스터형 인덱스가 성능 더 좋음
- 세컨더리 인덱스는 여러 개의 필드 값 기반으로 쿼리 많이 보낼 때 보조 인덱스로 사용 

#### MongoDB
- 자동으로 형성된 ObjectId가 기본키로 사용
- 세컨더리 키도 부가적으로 설정할 수 있음

### 인덱스 최적화 기법

#### 인덱스 비용 (MongoDB 기반 설명이지만 다른 것도 비슷함)
- 두번 탐색해야 함 (인덱스 리스트, 컬렉션)
- 컬렉션 수정되었을 때 인덱스도 수정되어야 함 (B-트리 조정,데이터 분산 비용)
- 따라서 쿼리의 모든 필드에 인덱스 설정하는 것이 좋은 것이 아님
- 컬렉션에서 가져와야 하는 양이 많을 수록 인덱스 사용이 비효율적

#### 테스팅하기
explain() 함수 통해 쿼리 걸리는 시간 최소화

#### 복합 인덱스 순서
여러 필드 기반 조회: 복합 인덱스 생성, 생성 순서에 따라 성능이 달라짐  
**같음, 정렬, 다중 값, 카디널리티 순서**
- 같음(== 등) 비교 ➡️ 정렬에 쓰이는 필드 ➡️ >, < 등 다중 값이 결과인 필드
- 카디널리티(unique한 정도)는 높은 것을 먼저
...
![R1280x0](https://github.com/CS-STUDY-17/CS-Study/assets/86969518/52644ee9-016a-4327-a133-565f1e7cc0ea)

### 조인의 원리
- 중첩 루프 조인(NLJ, Nested Loop Join)
  - 중첩 for문과 같은 원리
  - 블록 중첩 루프 조인(BNL, Block Nested Loop): 조인할 테이블을 작은 블록으로 나누어 블록 하나씩 조인
- 정렬 병합 조인
  - 조인할 필드 기준 각각 테이블 정렬 후 조인
  - 적절한 인덱스 없고 대용량 테이블 조인하고 >, < 등 범위 비교 연산자 있을 때 사용
- 해시 조인
  - 하나의 테이블이 메모리에 온전히 들어간다면 보통 중첩 루프 조인보다 효율적
  - 메모리에 안들어가면 디스크 사용 비용 발생
  - 동등(=) 조인에서만 사용 가능
  - 단계
    - 빌드 단계: 바이트 더 작은 테이블로 메모리 내 해시 테이블 만듦, 조인에 사용되는 필드가 해시 테이블의 키
    - 프로브 단계: 레코드 읽기, 일치 레코드 찾아 결괏값 반환